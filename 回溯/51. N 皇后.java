...
对于副对角线上的任意两个点 (r1, c1) 和 (r2, c2)，它们满足以下关系：
r1+c1=r2+c2r1+c1=r2+c2

换句话说，对于同一个副对角线上的点，它们的行索引加上列索引的值是相同的。
因此，我们可以用 row + col 来表示副对角线的唯一标识。

在代码中，我们检查当前要放置皇后的位置 (row, col) 是否已经在之前的某一行 i 上的副对角线上有皇后：
chessboard[i]+i==col+rowchessboard[i]+i==col+row
*/
